...
export class MessageService {
  ...
  async addNewMessageToCollection(
    ...
    try {
      ...
      if (attachments.length > 0)
        this.uploadAndAddAttachmentsToMessage(
          response.id,
          response.path,
          attachments
        );
   ...
   */
  private async uploadAndAddAttachmentsToMessage(
    messageID: string,
    messagePath: string,
    attachments: MessageAttachment[]
  ): Promise<void> {
    ...
    if (uploadedAttachments.length > 0)
      await updateDoc(doc(this.firestore, messagePath), {
    ...
    try {
      ...
      const updateData =
        collectionObject instanceof Message
          ...
          : { messagesCount: messagesQuerySnapshot.size };
      await updateDoc(
        doc(this.firestore, getObjectsPath(collectionObject)),
        updateData
      );
      return '';
    } catch (error) {
  ...
  private async deleteAllAnswersFromMessage(message: Message): Promise<string> {
    try {
      ...
      const deletePromises = answersQuerySnapshot.docs.map((doc) =>
        deleteDoc(doc.ref)
      );
  ...
  async updateMessage(
    ...
    try {
      if (updateData.content && updateData.content != message.content) {
        ...
        updateData.plainContent = removeAllHTMLTagsFromString(
          updateData.content
        );
  ...
  /**
   * Searches for messages that match the provided search query across all accessible channels and chats.
   *
   * @param searchQuery - The search query to match against message content.
   * @param messagesPath - The path to the message collection in Firestore. If empty, the service will search across all accessible channels and chats.
   ...
   */
  private async getMessagesFromQuerySnapshot(
    querySnapshot: any,
    searchLower: string
  ): Promise<Message[]> {
    ...
    querySnapshot.forEach((doc: any) => {
      ...
      const content = removeAllHTMLTagsFromString(
        messageData['content']
      ).toLowerCase();
